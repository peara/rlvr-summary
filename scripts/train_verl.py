...
def update_config_with_paths(
    config: OmegaConf, data_summary_path: Optional[str] = None
):
...
def setup_wandb_config(config: OmegaConf, wandb_config: dict):
    ...
    if wandb_config.get("enabled", False):
        ...
        logger.info(
            f"✅ W&B enabled: {config.trainer.project_name}/{config.trainer.experiment_name}"
        )
    else:
...
Examples:
  ...
  python scripts/train_verl.py trainer.total_epochs=2 data.train_batch_size=8
    ...
    parser.add_argument("--wandb-run", type=str, help="W&B run name")

    parser.add_argument(
        "--wandb-enabled", action="store_true", help="Enable W&B logging"
    )
    ...
    if not Path(config.custom_reward_function.path).exists():
        logger.error(
            f"❌ Reward function not found: {config.custom_reward_function.path}"
        )
        sys.exit(1)
    ...
    print(
        f"🎯 Total training steps: {config.actor_rollout_ref.actor.optim.total_training_steps}"
    )
