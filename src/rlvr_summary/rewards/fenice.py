...
class FENICEScorer(BaseRule):
    ...
    def evaluate(
        ...
        try:
            ...
            if cache_data:
                ...
                self.logger.warning(
                    "Cache data validation failed, falling back to runtime computation"
                )

            # Get FENICE results with or without cache
            if validated_cache:
                results = self._fenice_model.score_batch(
                    batch, document_cache_data=validated_cache
                )
            else:
    ...
    def batch_evaluate(
        ...
        try:
            ...
            if cache_data:
                fenice_results = self._fenice_model.score_batch(
                    batch, document_cache_data=cache_data
                )
            else:
