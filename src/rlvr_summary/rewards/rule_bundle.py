...
class RuleBundleRewardSystem:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.logger.info(f"RuleBundleRewardSystem initialized with {len(self.rules)} rules")
    ...
    def _setup_rules(self) -> None:
        ...
        if "fluency" in weights:
            self.rules["fluency"] = FluencyRule(
                ...
                config=self.config.get("fluency", {})
            )
    ...
    def _validate_config(self) -> None:
        ...
        for rule_name in weights:
            if rule_name not in self.rules:
                self.logger.warning(f"Rule '{rule_name}' has weight but no implementation")
    ...
    def evaluate_batch(
        ...
        if len(sources) != len(summaries):
            raise ValueError(f"Mismatch in batch sizes: {len(sources)} sources, {len(summaries)} summaries")
    ...
    def update_rule_weights(self, new_weights: Dict[str, float]) -> None:
        ...
        for rule_name, weight in new_weights.items():
            if rule_name in self.rules:
                ...
                self.logger.warning(f"Cannot update weight for unknown rule: '{rule_name}'")
...
def load_rule_bundle_from_config(config_path: Union[str, Path]) -> RuleBundleRewardSystem:
