...
class RewardSystemIntegrator:
                ...
                try:
                    ...
                    if hasattr(cfg, 'rewards'):
                        ...
                        if hasattr(reward_config, '_target_'):
                            ...
                            reward_dict = {k: v for k, v in reward_config.items() if k != '_target_'}
                        else:
    ...
    def compute_reward_batch(
        ...
        """
        results = self.reward_system.evaluate_batch(sources, summaries, log_details=log_details)
    ...
    def _compute_batch_metrics(self, results) -> Dict[str, float]:
        ...
        metrics = {
            ...
            "reward/batch_avg_pass_rate": sum(r.pass_rate for r in results) / batch_size,
        }
        
        # Add per-rule batch averages
        rule_names = list(results[0].rule_scores.keys())
        for rule_name in rule_names:
            avg_score = sum(r.rule_scores.get(rule_name, 0.0) for r in results) / batch_size
            avg_pass_rate = sum(float(r.rule_passed.get(rule_name, False)) for r in results) / batch_size
    ...
    def evaluate_milestone_criteria(self, target_pass_rate: float = 0.2) -> Dict[str, Any]:
        ...
        return {
            ...
            "message": (
                ...
                if milestone_met else
                f"❌ Milestone not yet met. Pass rate {current_pass_rate:.3f} < {target_pass_rate:.3f}"
            ),
        }
...
def create_reward_integrator(
    ...
    """
    if config_path:
        return RewardSystemIntegrator(config_path=config_path, wandb_logger=wandb_logger)
    else:
