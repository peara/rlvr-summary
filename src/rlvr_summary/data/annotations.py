                    ...
                    "properties": {
                        ...
                        "tool_calls": {
                            ...
                            "items": {
                                "type": "object",
                                "properties": {
                                    "tool": {"type": "string", "enum": ["search", "delete"]},
                                    "query": {"type": "string"},
                                    "position": {"type": "integer"},
                                    "results": {"type": "array"}
                                },
            ...
            if "annotations" not in annotation:
                ...
                if "quality_score" in annotation_content:
                    ...
                    if not isinstance(score, (int, float)) or not (0 <= score <= 1):
                        result["errors"].append("quality_score must be a number between 0 and 1")
                    ...
                    if not isinstance(errors, list):
                        ...
                        for i, error in enumerate(errors):
                            if not isinstance(error, dict):
                                result["errors"].append(f"factual_errors[{i}] must be an object")
                                continue
                            if "text" not in error or "error_type" not in error:
                                result["errors"].append(f"factual_errors[{i}] missing required fields")
                    ...
                    if not isinstance(calls, list):
                        ...
                        for i, call in enumerate(calls):
                            if not isinstance(call, dict):
                                result["errors"].append(f"tool_calls[{i}] must be an object")
                                continue
                            if "tool" not in call or "query" not in call:
                                result["errors"].append(f"tool_calls[{i}] missing required fields")
                            elif call["tool"] not in ["search", "delete"]:
                                result["errors"].append(f"tool_calls[{i}] invalid tool type: {call['tool']}")
    ...
    def merge_annotations(self, base_annotation: Dict, update_annotation: Dict) -> Dict:
        ...
        if "annotations" in update_annotation:
            ...
            for key, value in update_annotations.items():
                if key in ["factual_errors", "tool_calls"] and key in merged_annotations:
                    # Merge lists
                    if isinstance(value, list) and isinstance(merged_annotations[key], list):
                        merged_annotations[key].extend(value)
                    else:
                        merged_annotations[key] = value
                elif key == "metadata" and key in merged_annotations:
                    # Merge metadata objects
                    if isinstance(value, dict) and isinstance(merged_annotations[key], dict):
    ...
    def filter_annotations(
        ...
        for annotation in annotations:
            ...
            if error_types is not None and include:
                ...
                if not any(error.get("error_type") in error_types for error in factual_errors):
                    include = False
