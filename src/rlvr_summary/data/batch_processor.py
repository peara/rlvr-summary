...
class BatchProcessor:
        ...
        try:
            for item in data_iterator:
                ...
                if len(batch) >= self.batch_size:
                    ...
                    if (
                        self.save_interval
                        and self.stats["batches_processed"] % self.save_interval == 0
                    ):
        ...
        try:
            if parallel and self.max_workers and self.max_workers > 1:
                return self._process_batch_parallel(
                    batch, process_func, **process_kwargs
                )
            else:
    ...
    def _process_batch_sequential(
        self, batch: List[Any], process_func: Callable, **process_kwargs
    ) -> List[Any]:
    ...
    def get_progress_info(self) -> Dict[str, Any]:
        ...
        if progress["processing_time"] > 0:
            progress["items_per_second"] = (
                progress["processed_items"] / progress["processing_time"]
            )
        else:
...
def create_data_pipeline(
    ...
    try:
        ...
        def pipeline_process_func(item):
            ...
            if preprocessor:
                try:
                    ...
                    logger.warning(
                        f"Preprocessing failed for item {item.get('id', 'unknown')}: {e}"
                    )
            ...
            if validator:
                try:
                    ...
                    logger.warning(
                        f"Validation failed for item {item.get('id', 'unknown')}: {e}"
                    )
        ...
        pipeline_results = list(
            batch_processor.process_iterator(data_iterator, pipeline_process_func)
        )
        ...
        if output_path:
            ...
            logger.info(
                f"Saved {len(pipeline_results)} processed samples to {output_path}"
            )
    ...
    return {
        ...
        "loader_info": (
            loader.get_dataset_info() if hasattr(loader, "get_dataset_info") else {}
        ),
