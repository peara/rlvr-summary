...
class DataValidator:
        ...
        for field in text_fields:
            if field in sample and sample[field] is not None:
                if not isinstance(sample[field], str):
                    errors.append(
                        f"Field '{field}' must be string, got {type(sample[field])}"
                    )
    ...
    def _validate_article(self, article: str) -> List[str]:
        ...
        if article_length < self.min_article_length:
            errors.append(
                f"Article too short: {article_length} < {self.min_article_length}"
            )

        if article_length > self.max_article_length:
            errors.append(
                f"Article too long: {article_length} > {self.max_article_length}"
            )
    ...
    def _validate_summary(self, summary: str) -> List[str]:
        ...
        if summary_length < self.min_summary_length:
            errors.append(
                f"Summary too short: {summary_length} < {self.min_summary_length}"
            )

        if summary_length > self.max_summary_length:
            errors.append(
                f"Summary too long: {summary_length} > {self.max_summary_length}"
            )
    ...
    def _check_encoding_issues(self, sample: Dict) -> List[str]:
        ...
        for field in text_fields:
            if field in sample and sample[field]:
                ...
                if "�" in text:
                    warnings.append(
                        f"Field '{field}' contains replacement characters (encoding issues)"
                    )
                ...
                if non_ascii_count > len(text) * 0.3:  # More than 30% non-ASCII
                    warnings.append(
                        f"Field '{field}' has high non-ASCII character ratio"
                    )
        ...
        for field in text_fields:
            if field in sample and sample[field]:
                ...
                if total_unique_words > 10:  # Only check if we have enough words
                    english_ratio = english_word_count / min(
                        total_unique_words, len(english_indicators)
                    )
    ...
    def _check_duplicates(self, sample: Dict) -> List[str]:
        ...
        if "article" in sample and "highlights" in sample:
            ...
            if (
                highlight_words
                and len(highlight_words.intersection(article_words))
                / len(highlight_words)
                > 0.9
            ):
    ...
    def batch_validate(self, samples: List[Dict]) -> Dict[str, Any]:
        ...
        if batch_stats["total_samples"] > 0:
            batch_stats["validation_rate"] = (
                batch_stats["valid_samples"] / batch_stats["total_samples"]
            )
