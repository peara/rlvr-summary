...
class ModelManager:
    ...
    def get_claim_extractor(self, batch_size: int = 256, device: str = None):
        ...
        if (self._claim_extractor is None or 
            ...
            self.logger.info(f"Loading ClaimExtractor (device={device}, batch_size={batch_size})")
    ...
    def get_nli_aligner(self, batch_size: int = 256, device: str = None, max_length: int = 1024):
        ...
        # Check if we need to create/recreate the model
        if (self._nli_aligner is None or 
            self._nli_aligner_config != config):
            
            self.logger.info(f"Loading NLIAligner (device={device}, batch_size={batch_size}, max_length={max_length})")
    ...
    def get_coref_model(self, batch_size: int = 1, device: str = None, load_model: bool = True):
        ...
        # Check if we need to create/recreate the model
        if (self._coref_model is None or 
            self._coref_model_config != config):
            
            self.logger.info(f"Loading CoreferenceResolution (device={device}, batch_size={batch_size})")
