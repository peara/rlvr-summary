...
class TestBartMNLIConsistencyRule:
    ...
    def test_initialization_custom_config(self):
        ...
        with patch('rlvr_summary.rewards.bart_mnli.AutoModelForSequenceClassification') as mock_model:
            with patch('rlvr_summary.rewards.bart_mnli.AutoTokenizer') as mock_tokenizer:
    ...
    def test_batch_evaluate(self):
        ...
        mock_results = [
            {"labels": ["entailed by source", "not entailed by source"], "scores": [0.9, 0.1]},
            {"labels": ["entailed by source", "not entailed by source"], "scores": [0.3, 0.7]},
        ]
    ...
    def test_manual_pytorch_mode_mock(self):
        ...
        with patch('rlvr_summary.rewards.bart_mnli.AutoModelForSequenceClassification') as mock_model_class:
            with patch('rlvr_summary.rewards.bart_mnli.AutoTokenizer') as mock_tokenizer_class:
                mock_model_class.from_pretrained.return_value = mock_model
                mock_tokenizer_class.from_pretrained.return_value = mock_tokenizer
                
                rule = BartMNLIConsistencyRule(config={
                    "threshold": 0.6,
                    "use_pipeline": False,
                    "device": "cpu"
                })
                ...
                assert abs(result["details"]["entailment_probability"] - expected_prob) < 0.01
